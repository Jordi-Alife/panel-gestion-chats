...
const ConversacionesMovilArchivadas = () => {
  ...
  cargarDatos(); // Solo una vez al montar
}, []);
  ...
  info.estado !== "cerrado" &&
  !localStorage.getItem(`liberada-${id}`)
) {
  fetch(`${BACKEND_URL}/api/liberar-conversacion`, {
    ...
    .then(() => {
      ...
      localStorage.setItem(`liberada-${id}`, "true");
    })
    .catch((err) => {
...
<div className="flex justify-center gap-2 px-4 py-2 border-b">
  <button
    ...
    className="relative text-xs font-medium px-3 py-1 rounded-full bg-gray-200 text-gray-600"
  >
    Recientes
  </button>
