...
export default function Detalle() {
  ...
  const conversacionesPorUsuario = conversaciones.reduce((acc, item) => {
  ...
  return (
    <div className="flex h-screen bg-[#f0f4f8] overflow-hidden">
      {/* Columna izquierda */}
      <div className="w-[20%] p-4 bg-white border-r overflow-y-auto">
        <h3 className="text-sm text-gray-500 font-semibold mb-2">Conversaciones</h3>
        <div className="space-y-2">
          {Object.entries(conversacionesPorUsuario).map(([id, info]) => {
            const nuevos = info.mensajes.filter(
              (m) =>
                m.from === "usuario" &&
                (!vistas[id] || new Date(m.lastInteraction) > new Date(vistas[id]))
            ).length;

            return (
              <div
                key={id}
                className={`cursor-pointer p-2 rounded hover:bg-blue-50 border ${id === userId ? 'bg-blue-100 border-blue-400' : ''}`}
              ...
              >
                ...
                <div className="text-xs text-gray-500">
                  {formatearTiempo(info.lastInteraction)}
                </div>
              </div>
            );
          })}
      ...
      <div className="flex-1 flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="sticky bottom-0 bg-white border-t flex items-center px-4 py-3 space-x-2">
          ...
          />
          <button type="submit" className="bg-blue-600 text-white rounded-full px-4 py-2 text-sm hover:bg-blue-700">
            Enviar
          </button>
        </form>
      </div>
