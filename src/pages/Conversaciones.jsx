...
export default function Conversaciones() {
  ...
  const cargarDatos = () => {
    ...
    fetch("https://web-production-51989.up.railway.app/api/vistas")
      .then((res) => res.json())
      .then((data) => setVistas(data.vistasPorAgente || {}))
      .catch(console.error);
  };
  ...
  const handleSendEmail = async () => {
    ...
    await fetch("https://web-production-51989.up.railway.app/api/enviar-email", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  const listaAgrupada = Object.entries(conversacionesPorUsuario)
    .map(([id, info]) => {
      const claveVista = `${id}_${perfil.nombre}`;
      const ultimaVista = vistas[claveVista];
      const nuevos = info.intervenida && info.intervenidaPor?.nombre === perfil?.nombre
  ...
  return (
    <div className="flex flex-col h-[100dvh] bg-[#f0f4f8] relative">
      <div className="flex flex-1 p-4 gap-4 overflow-hidden h-[calc(100dvh-5.5rem)]">
          ...
          <div className="flex gap-2 mb-3">
            {["todas", "gpt", "humanas"].map(f => (
              <button
                key={f}
                onClick={() => setFiltro(f)}
                className={`text-xs px-2 py-1 rounded-full border ${filtro === f ? "bg-blue-600 text-white" : "bg-white text-gray-700"}`}
              >
        ...
        {/* Aquí irían las columnas central y derecha (idénticas a las anteriores) */}
        {/* ... */}

        {/* BLOQUE FINAL: Enviar conversación por email */}
      </div>
