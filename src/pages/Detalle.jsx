    ...
    <div className="flex flex-col h-[100dvh] bg-[#f0f4f8]">
      <div className="flex flex-1 p-4 gap-4 overflow-hidden h-[calc(100dvh-5.5rem)]">
        ...
        <div className="w-1/5 bg-white rounded-lg shadow-md p-4 overflow-y-auto h-full">
          ...
          {listaAgrupada.map((c) => (
            <div key={c.userId} onClick={() => navigate(`/conversacion/${c.userId}`)}
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-500">{c.lastInteraction ? new Date(c.lastInteraction).toLocaleTimeString() : ''}</div>
                </div>
              </div>
              <span className={`text-[10px] text-white px-2 py-0.5 rounded-full ${estadoColor[c.estado] || 'bg-gray-400'}`}>{c.estado}</span>
            </div>
          ))}
        </div>
        ...
        <div className="flex-1 bg-white rounded-lg shadow-md flex flex-col overflow-hidden h-full relative">
          <div ref={chatRef} onScroll={handleScroll} className="flex-1 overflow-y-auto p-6 space-y-4">
            {mensajes.map((msg, index) => (
              <div key={index} className={`flex ${msg.from === 'asistente' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] p-4 rounded-2xl shadow-md ${msg.from === 'asistente' ? 'bg-[#ff5733] text-white' : 'bg-white text-gray-800 border'}`}>
                  {esURLImagen(msg.message) ? (
                    <img src={msg.message} alt="Imagen" className="rounded-lg max-w-full max-h-[300px] object-contain" />
                  ) : (
          ...
          <form onSubmit={handleSubmit} className="border-t px-4 py-3 flex flex-col gap-2 sm:flex-row sm:items-center">
            ...
            <input type="text" value={respuesta} onChange={(e) => setRespuesta(e.target.value)} placeholder="Escribe un mensaje..."
      ...
      {/* Formulario de enviar conversación por email */}
