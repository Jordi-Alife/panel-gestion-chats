...
export default function Detalle() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!userId || (!respuesta.trim() && !imagen)) return;
  ...
  const formatearTiempo = (fecha) => {
    ...
    if (diffMin < 1) return `hace ${Math.floor(diffMs / 1000)}s`;
  ...
  const listaAgrupada = Object.entries(conversacionesPorUsuario).map(([id, info]) => {
    ...
    const nuevos = info.mensajes.filter(m => (!ultimaVista || new Date(m.lastInteraction) > new Date(ultimaVista))).length;
    ...
    if (info.estado === "cerrada") estado = "Cerrado";
    else if (mensajesUsuario.length === 1 && minutosDesdeUltimo < 2) estado = "Nuevo";
    else if (mensajesUsuario.length > 1 && minutosDesdeUltimo < 2) estado = "Activo";
    else estado = "Inactivo";

    return { userId: id, nuevos, estado, lastInteraction: info.lastInteraction, iniciales: id.slice(0, 2).toUpperCase() };
  });
              ...
              return (
                <div key={index} className={`flex ${align}`}>
                  <div className={`max-w-[80%] p-4 rounded-2xl shadow-md ${bubbleColor}`}>
                    ...
                    <div className="text-[10px] mt-1 opacity-60 text-right">{new Date(msg.lastInteraction).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</div>
                  </div>
                </div>
              );
            })}
